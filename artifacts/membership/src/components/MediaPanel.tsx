...
export function MediaPanel({ media }: { media: LessonMedia[] }) {
  ...
  return (
    <aside className="flex h-fit flex-col gap-6 rounded-sm border border-line bg-subtle p-5">
      {tracks.length > 0 && (
        <section>
          <h3 className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-champagne">
            <Music size={13} aria-hidden="true" /> Audio
          </h3>
          <ul className="flex flex-col gap-4">
            {tracks.map((m) => (
              <li key={m.media.id}>
                <p className="mb-1.5 text-sm text-charcoal">{m.media.title ?? "Track"}</p>
                {!m.media.available ? (
                  <Unavailable />
                ) : urls[m.media.id] ? (
                  <audio controls preload="none" src={urls[m.media.id]!} className="w-full" />
                ) : (
                  <p className="text-xs text-mute">Preparing…</p>
                )}
              </li>
            ))}
          </ul>
          ...
          <ul className="flex flex-col gap-2">
            {worksheets.map((m) => (
              <li key={m.media.id}>
                {!m.media.available ? (
                  <div>
                    <p className="text-sm text-mute line-through">{m.media.title ?? "Worksheet"}</p>
                    <Unavailable />
                  </div>
                ) : (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block rounded-sm border border-line bg-surface px-3 py-2 text-sm text-aubergine no-underline hover:border-champagne"
                  >
